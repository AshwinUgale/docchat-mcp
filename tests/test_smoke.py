...
verification is end-to-end via `docchat-mcp index react 18.2.0` against
...
def test_package_imports() -> None:
    ...
    assert docchat_mcp.__version__ == "0.0.1"
...
def test_urls_for_unknown_library_returns_empty() -> None:
    from docchat_mcp.library_config import urls_for
...
def test_default_qdrant_path_under_home_dir() -> None:
    from docchat_mcp.indexer import default_qdrant_path

    p = default_qdrant_path()
    assert isinstance(p, Path)
    assert ".docchat-mcp" in str(p)


def test_qdrant_path_env_override(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    from docchat_mcp.indexer import default_qdrant_path

    monkeypatch.setenv("QDRANT_PATH", "/tmp/test-docchat-mcp-qdrant")
    assert default_qdrant_path() == Path("/tmp/test-docchat-mcp-qdrant")
...
def test_cli_help_runs() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "docchat_mcp.cli", "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_indexer_raises_on_unknown_library() -> None:
    from docchat_mcp.indexer import DocIndexer
